...
def billing_dashboard(request):
    ...
    # Base queryset: exclude invoices for patients belonging to a group
    ...
    outstanding_amount = _indiv.filter(
    ...
    overdue_count = _indiv.filter(status='overdue').count()
    ...
    # Combined recent and draft invoices (last 15) - exclude group members
    all_invoices = _indiv.select_related('patient').order_by('-created_at')[:15]
    ...
    overdue_invoices = _indiv.filter(status='overdue').select_related('patient')[:5]
    ...
    # Regular invoice aging (exclude group members)
    regular_aging = [
        _indiv.filter(status__in=['sent', 'overdue'], due_date__gte=today).aggregate(Sum('total_amount'))['total_amount__sum'] or 0,
        _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today, due_date__gte=today - timedelta(days=30)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0,
        _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=30), due_date__gte=today - timedelta(days=60)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0,
        _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=60), due_date__gte=today - timedelta(days=90)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0,
        _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=90)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0,
    ]
...
def invoice_create_for_patient(request):
    ...
    if request.method == 'POST':
        ...
        if patient_id:
            ...
            # Block individual invoices for patients belonging to a group
            if patient.patient_group_id:
                msg = (f'{patient.get_full_name()} belongs to group '
                       f'"{patient.patient_group.name}". '
                       f'Please use Group Invoicing instead.')
...
def invoice_create_ajax(request):
    ...
    if form.is_valid():
        # Block individual invoices for group patients
        patient = form.cleaned_data.get('patient')
        if patient and patient.patient_group_id:
            return JsonResponse({
                'success': False,
                'errors': {'patient': [
                    f'{patient.get_full_name()} belongs to group '
                    f'"{patient.patient_group.name}". '
                    f'Please use Group Invoicing instead.'
                ]}
            }, status=400)
...
def invoice_create_full_ajax(request):
    ...
    # Block individual invoices for group patients
    patient = header_form.cleaned_data.get('patient')
    if patient and patient.patient_group_id:
        return JsonResponse({
            'success': False,
            'errors': {'patient': [
...
def invoice_list(request):
    ...
    # Get all invoices for statistics (exclude group-member invoices)
    all_invoices = Invoice.objects.select_related('patient').exclude(patient__patient_group__isnull=False)
    ...
    for ginv in group_invoices:
        if ginv.status == 'paid':
            group_paid_amount += ginv.total_amount
        elif ginv.status in ['sent', 'overdue', 'partial']:
            group_outstanding_amount += ginv.get_balance_due()
    ...
    group_stats = {
        ...
        'group_cancelled_count': group_invoices.filter(status='cancelled').count(),
        'group_paid_amount': group_paid_amount,
        'group_outstanding': group_outstanding_amount,
    }
    
    # Combined statistics
    combined_stats = {
        'total_all_invoices': stats['total_invoices'] + group_stats['total_group_invoices'],
        'total_all_amount': float(stats['total_amount']) + float(group_stats['group_total_amount']),
        'total_outstanding': float(stats['outstanding_amount']) + float(group_stats['group_outstanding']),
        'total_overdue': stats['overdue_count'] + group_stats['group_overdue_count'],
        'total_paid_amount': float(stats['paid_amount']) + float(group_stats['group_paid_amount']),
        'total_paid_count': stats['paid_count'] + group_stats['group_paid_count'],
        'total_draft_count': stats['draft_count'] + group_stats['group_draft_count'],
        'total_sent_count': stats['sent_count'] + group_stats['group_sent_count'],
        'total_cancelled_count': stats['cancelled_count'] + group_stats['group_cancelled_count'],
    }
    ...
    # Group invoices list (apply same filters where applicable)
    group_invoices_qs = GroupInvoice.objects.select_related('patient_group', 'created_by').all()
    if status_filter:
        group_invoices_qs = group_invoices_qs.filter(status=status_filter)
    if search_query:
        group_invoices_qs = group_invoices_qs.filter(
            Q(invoice_number__icontains=search_query) |
            Q(patient_group__name__icontains=search_query)
        )
    if date_from:
        group_invoices_qs = group_invoices_qs.filter(issue_date__gte=date_from)
    if date_to:
        group_invoices_qs = group_invoices_qs.filter(issue_date__lte=date_to)
    if amount_min:
        try:
            group_invoices_qs = group_invoices_qs.filter(total_amount__gte=float(amount_min))
        except ValueError:
            pass
    if amount_max:
        try:
            group_invoices_qs = group_invoices_qs.filter(total_amount__lte=float(amount_max))
        except ValueError:
            pass
    group_invoices_list = group_invoices_qs.order_by('-created_at')

    context = {
        'page_obj': pagination_data['page_obj'],
        'invoices': pagination_data['items'],
        'group_invoices': group_invoices_list,
...
def invoice_aging_report(request):
    ...
    # Regular invoices (exclude group-member invoices)
    _indiv = Invoice.objects.exclude(patient__patient_group__isnull=False)
    current_invoices = _indiv.filter(
        status__in=['sent', 'overdue'],
        due_date__gte=today
    )
    
    overdue_1_30 = _indiv.filter(
        status__in=['sent', 'overdue'],
        due_date__lt=today,
        due_date__gte=today - timedelta(days=30)
    )
    
    overdue_31_60 = _indiv.filter(
        status__in=['sent', 'overdue'],
        due_date__lt=today - timedelta(days=30),
        due_date__gte=today - timedelta(days=60)
    )
    
    overdue_61_90 = _indiv.filter(
        status__in=['sent', 'overdue'],
        due_date__lt=today - timedelta(days=60),
        due_date__gte=today - timedelta(days=90)
    )
    
    overdue_90_plus = _indiv.filter(
        status__in=['sent', 'overdue'],
...
def patient_draft_invoices(request, patient_id):
    ...
    # Group members have no individual invoices
    if patient.patient_group_id:
        draft_invoices = Invoice.objects.none()
    else:
...
def invoices_for_patient_ajax(request):
    ...
    # Group members have no individual invoices
    if patient.patient_group_id:
        return JsonResponse({'invoices': [], 'group': patient.patient_group.name})
...
def export_invoices_csv(request):
    ...
    invoices = Invoice.objects.select_related('patient').exclude(patient__patient_group__isnull=False)
...
def billing_reports(request):
    ...
    # Aging summary - include both regular (non-group) and group invoices
    _indiv = Invoice.objects.exclude(patient__patient_group__isnull=False)
    aging_summary = {
        'current': {
            'invoices': _indiv.filter(status__in=['sent', 'overdue'], due_date__gte=today),
            'total': (_indiv.filter(status__in=['sent', 'overdue'], due_date__gte=today).aggregate(Sum('total_amount'))['total_amount__sum'] or 0) +
                     (GroupInvoice.objects.filter(status__in=['sent', 'overdue'], due_date__gte=today).aggregate(Sum('total_amount'))['total_amount__sum'] or 0),
        },
        'overdue_1_30': {
            'invoices': _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today, due_date__gte=today - timedelta(days=30)),
            'total': (_indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today, due_date__gte=today - timedelta(days=30)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0) +
                     (GroupInvoice.objects.filter(status__in=['sent', 'overdue'], due_date__lt=today, due_date__gte=today - timedelta(days=30)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0),
        },
        'overdue_31_60': {
            'invoices': _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=30), due_date__gte=today - timedelta(days=60)),
            'total': (_indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=30), due_date__gte=today - timedelta(days=60)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0) +
                     (GroupInvoice.objects.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=30), due_date__gte=today - timedelta(days=60)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0),
        },
        'overdue_61_90': {
            'invoices': _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=60), due_date__gte=today - timedelta(days=90)),
            'total': (_indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=60), due_date__gte=today - timedelta(days=90)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0) +
                     (GroupInvoice.objects.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=60), due_date__gte=today - timedelta(days=90)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0),
        },
        'overdue_90_plus': {
            'invoices': _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=90)),
            'total': (_indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=90)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0) +
                     (GroupInvoice.objects.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=90)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0),
        },
    }

    # Top patients by outstanding balance (exclude group members)
    top_debtors = (
        _indiv.filter(status__in=['sent', 'overdue'])
    ...
    # Overall stats - include both regular (non-group) and group invoices/payments
    regular_invoiced = _indiv.aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    ...
    regular_outstanding = _indiv.filter(status__in=['sent', 'overdue']).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    ...
    overall_stats = {
        ...
        'total_invoices': _indiv.count() + GroupInvoice.objects.count(),
        'paid_invoices': _indiv.filter(status='paid').count() + GroupInvoice.objects.filter(status='paid').count(),
        'overdue_invoices': _indiv.filter(status='overdue').count() + GroupInvoice.objects.filter(status='overdue').count(),
    ...
    # ---- PER-PATIENT BILLING SUMMARY (exclude group members) ----
    patient_summary = (
        _indiv
        .values('patient__id', 'patient__first_name', 'patient__last_name')
        .annotate(
