...
def reports_dashboard(request):
    ...
    # Outstanding balance - calculate from unpaid/partially paid invoices (exclude group members)
    unpaid_invoices = Invoice.objects.exclude(
        patient__patient_group__isnull=False
    ).filter(
...
def _get_patient_billing_stats(patients, start_date, end_date):
    ...
    inv_q = Invoice.objects.exclude(patient__patient_group__isnull=False).filter(patient__in=patients, created_at__date__range=[start_date, end_date])
    ...
    for i in range(6):
        ms = (end_date.replace(day=1) - timedelta(days=i * 30)).replace(day=1)
        me = (ms + timedelta(days=32)).replace(day=1) - timedelta(days=1)
        b = Invoice.objects.exclude(patient__patient_group__isnull=False).filter(patient__in=patients, created_at__date__range=[ms, me]).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
...
def _get_billing_stats(start_date, end_date, prev_start, prev_end):
    ...
    from billing.models import InsuranceClaim, GroupInvoice, GroupPayment
    from django.db.models import F
    ...
    indiv_revenue = payments_query.aggregate(Sum('amount'))['amount__sum'] or 0

    # Group payments
    group_payments_query = GroupPayment.objects.filter(
        payment_date__date__range=[start_date, end_date], status='completed'
    )
    group_revenue = group_payments_query.aggregate(Sum('amount'))['amount__sum'] or 0

    services_revenue = float(indiv_revenue) + float(group_revenue)
    payments_received = services_revenue

    _indiv = Invoice.objects.exclude(patient__patient_group__isnull=False)
    invoices_query = _indiv.filter(created_at__date__range=[start_date, end_date])
    group_invoices_query = GroupInvoice.objects.filter(created_at__date__range=[start_date, end_date])
    total_invoices = invoices_query.count() + group_invoices_query.count()
    indiv_avg = invoices_query.aggregate(Avg('total_amount'))['total_amount__avg'] or 0
    avg_invoice_value = indiv_avg

    # Outstanding: individual + group
    indiv_outstanding = _indiv.filter(status__in=['sent', 'overdue']).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    group_outstanding = GroupInvoice.objects.filter(status__in=['sent', 'overdue']).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    outstanding_amount = float(indiv_outstanding) + float(group_outstanding)
    collection_rate = (float(payments_received) / float(services_revenue) * 100) if services_revenue > 0 else 0

    # Previous period revenue (individual + group)
    prev_indiv = Payment.objects.filter(
        payment_date__date__range=[prev_start, prev_end], status='completed'
    ).aggregate(Sum('amount'))['amount__sum'] or 0
    prev_group = GroupPayment.objects.filter(
        payment_date__date__range=[prev_start, prev_end], status='completed'
    ).aggregate(Sum('amount'))['amount__sum'] or 0
    prev_revenue = float(prev_indiv) + float(prev_group)
    ...
    # Outstanding aging
    today = date.today()
    o30 = _indiv.filter(status__in=['sent', 'overdue'], due_date__gte=today - timedelta(days=30)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    o60 = _indiv.filter(status__in=['sent', 'overdue'], due_date__range=[today - timedelta(days=60), today - timedelta(days=31)]).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    o90 = _indiv.filter(status__in=['sent', 'overdue'], due_date__range=[today - timedelta(days=90), today - timedelta(days=61)]).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    o90p = _indiv.filter(status__in=['sent', 'overdue'], due_date__lt=today - timedelta(days=90)).aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    tot_out = o30 + o60 + o90 + o90p

    # Monthly revenue (current & prev year) – include group payments
    monthly_labels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    cy = date.today().year
    current_year_data = []
    previous_year_data = []
    for m in range(1, 13):
        indiv_cur = float(Payment.objects.filter(payment_date__year=cy, payment_date__month=m, status='completed').aggregate(Sum('amount'))['amount__sum'] or 0)
        grp_cur = float(GroupPayment.objects.filter(payment_date__year=cy, payment_date__month=m, status='completed').aggregate(Sum('amount'))['amount__sum'] or 0)
        current_year_data.append(indiv_cur + grp_cur)
        indiv_prev = float(Payment.objects.filter(payment_date__year=cy - 1, payment_date__month=m, status='completed').aggregate(Sum('amount'))['amount__sum'] or 0)
        grp_prev = float(GroupPayment.objects.filter(payment_date__year=cy - 1, payment_date__month=m, status='completed').aggregate(Sum('amount'))['amount__sum'] or 0)
        previous_year_data.append(indiv_prev + grp_prev)

    # Revenue trend (daily) – include group payments
    period_days = (end_date - start_date).days + 1
    trend_labels = []
    trend_data = []
    for i in range(min(period_days, 90)):
        day = start_date + timedelta(days=i)
        indiv_day = float(Payment.objects.filter(payment_date__date=day, status='completed').aggregate(Sum('amount'))['amount__sum'] or 0)
        grp_day = float(GroupPayment.objects.filter(payment_date__date=day, status='completed').aggregate(Sum('amount'))['amount__sum'] or 0)
        trend_labels.append(day.strftime('%m/%d'))
        trend_data.append(indiv_day + grp_day)
    ...
    # Invoices by status (exclude group members)
    inv_status = _indiv.filter(created_at__date__range=[start_date, end_date]).values('status').annotate(cnt=Count('id'))
...
def _get_pharmacy_financial_stats(start_date, end_date):
    """Gather pharmacy FINANCIAL statistics only (revenue, costs, profit)."""
    from pharmacy.models import StockMovement, Batch, Medication, PurchaseOrder
    from django.db.models import F
    ...
    sales_revenue = float(sales_q.annotate(rev=F('quantity') * F('batch__selling_price')).aggregate(total=Sum('rev'))['total'] or 0)
    sales_cogs = float(sales_q.annotate(cost=F('quantity') * F('batch__cost_price')).aggregate(total=Sum('cost'))['total'] or 0)
    sales_profit = sales_revenue - sales_cogs
    sales_margin = (sales_profit / sales_revenue * 100) if sales_revenue > 0 else 0
    total_items_sold = sales_q.aggregate(Sum('quantity'))['quantity__sum'] or 0
    total_transactions = sales_q.count()
    ...
    po_q = PurchaseOrder.objects.filter(order_date__range=[start_date, end_date])
    po_count = po_q.count()
    po_total = float(po_q.aggregate(Sum('total_amount'))['total_amount__sum'] or 0)

    # Top selling medications by revenue
    top_meds = sales_q.values('batch__medication__name').annotate(
        qty=Sum('quantity'),
        rev=Sum(F('quantity') * F('batch__selling_price')),
        cost=Sum(F('quantity') * F('batch__cost_price')),
    ).order_by('-rev')[:10]
    top_meds_list = []
    for m in top_meds:
        r = float(m['rev'] or 0)
        ...
        top_meds_list.append({
            'name': m['batch__medication__name'] or 'Unknown',
            'qty': m['qty'], 'revenue': r, 'cost': c, 'profit': r - c,
        })
    ...
    for i in range(6):
        ms = (end_date.replace(day=1) - timedelta(days=i * 30)).replace(day=1)
        me = (ms + timedelta(days=32)).replace(day=1) - timedelta(days=1)
        mq = StockMovement.objects.filter(
            created_at__date__range=[ms, me], movement_type='out', reference__icontains='SALE'
        )
        rev = float(mq.annotate(r=F('quantity') * F('batch__selling_price')).aggregate(t=Sum('r'))['t'] or 0)
        cog = float(mq.annotate(c=F('quantity') * F('batch__cost_price')).aggregate(t=Sum('c'))['t'] or 0)
        monthly_labels.insert(0, ms.strftime('%b %Y'))
        monthly_revenue.insert(0, rev)
        monthly_profit.insert(0, rev - cog)

    return {
        'sales_revenue': sales_revenue, 'sales_cogs': sales_cogs,
        'sales_profit': sales_profit, 'sales_margin': round(sales_margin, 1),
        'total_items_sold': total_items_sold, 'total_transactions': total_transactions,
        'po_count': po_count, 'po_total': po_total,
        'top_meds': top_meds_list,
        'monthly_labels': monthly_labels, 'monthly_revenue': monthly_revenue,
        'monthly_profit': monthly_profit,
    }


def _get_laboratory_financial_stats(start_date, end_date):
    """Gather laboratory FINANCIAL statistics only (revenue from tests)."""
    from laboratory.models import LabTestRequest, LabTest

    req_q = LabTestRequest.objects.filter(date_requested__date__range=[start_date, end_date])
    total_tests = req_q.count()
    completed_tests = req_q.filter(status='completed').count()

    # Revenue = sum of test prices for completed/billed requests
    total_revenue = 0
    revenue_by_category = {}
    top_tests_rev = {}
    for r in req_q.filter(status__in=['completed', 'sample_collected', 'in_progress', 'requested']).select_related('test', 'test__category'):
        price = float(r.test.price) if r.test else 0
        total_revenue += price
        # By category
        cat = r.test.category.name if r.test and r.test.category else 'Uncategorized'
        revenue_by_category[cat] = revenue_by_category.get(cat, 0) + price
        # By test name
        tname = r.test.name if r.test else 'Unknown'
        if tname not in top_tests_rev:
            top_tests_rev[tname] = {'name': tname, 'revenue': 0, 'count': 0}
        top_tests_rev[tname]['revenue'] += price
        top_tests_rev[tname]['count'] += 1

    # Sort top tests by revenue
    top_tests = sorted(top_tests_rev.values(), key=lambda x: x['revenue'], reverse=True)[:10]

    # Sort categories by revenue
    cat_sorted = sorted(revenue_by_category.items(), key=lambda x: x[1], reverse=True)
    cat_labels = [c[0] for c in cat_sorted]
    cat_data = [c[1] for c in cat_sorted]
    ...
    for i in range(6):
        ...
        me = (ms + timedelta(days=32)).replace(day=1) - timedelta(days=1)
        month_rev = 0
        for r in LabTestRequest.objects.filter(
            date_requested__date__range=[ms, me],
            status__in=['completed', 'sample_collected', 'in_progress', 'requested']
        ).select_related('test'):
            month_rev += float(r.test.price) if r.test else 0
        monthly_labels.insert(0, ms.strftime('%b %Y'))
        monthly_data.insert(0, month_rev)

    avg_test_value = (total_revenue / total_tests) if total_tests > 0 else 0

    return {
        'total_revenue': total_revenue, 'total_tests': total_tests,
        'completed_tests': completed_tests,
        'avg_test_value': round(avg_test_value, 0),
        'top_tests': top_tests,
...
def _quick_period_stats(s, e, enabled):
    """Compute revenue / expenses / net for a single date range."""
    from django.db.models import F
    from billing.models import GroupInvoice, GroupPayment

    # Billing revenue (completed payments – individual + group)
    svc = 0
    if 'billing' in enabled:
        indiv_svc = float(Payment.objects.filter(
            payment_date__date__range=[s, e], status='completed'
        ).aggregate(Sum('amount'))['amount__sum'] or 0)
        grp_svc = float(GroupPayment.objects.filter(
            payment_date__date__range=[s, e], status='completed'
        ).aggregate(Sum('amount'))['amount__sum'] or 0)
        svc = indiv_svc + grp_svc

    # Pharmacy revenue
    pharm = 0
    if 'pharmacy' in enabled:
        from pharmacy.models import StockMovement
        sq = StockMovement.objects.filter(
            created_at__date__range=[s, e], movement_type='out', reference__icontains='SALE'
        )
        pharm = float(sq.annotate(r=F('quantity') * F('batch__selling_price')).aggregate(t=Sum('r'))['t'] or 0)
    ...
    if 'laboratory' in enabled:
        ...
        for r in LabTestRequest.objects.filter(
            date_requested__date__range=[s, e],
            status__in=['completed', 'sample_collected', 'in_progress', 'requested']
        ).select_related('test'):
            lab += float(r.test.price) if r.test else 0
    ...
    if 'budget' in enabled:
        ...
        exp = float(Expense.objects.filter(
            expense_date__range=[s, e], status__in=['approved', 'paid']
        ).aggregate(Sum('amount'))['amount__sum'] or 0)

    total_rev = svc
    indiv_inv_count = Invoice.objects.exclude(patient__patient_group__isnull=False).filter(created_at__date__range=[s, e]).count() if 'billing' in enabled else 0
    grp_inv_count = GroupInvoice.objects.filter(created_at__date__range=[s, e]).count() if 'billing' in enabled else 0
    indiv_pay_count = Payment.objects.filter(payment_date__date__range=[s, e], status='completed').count() if 'billing' in enabled else 0
    grp_pay_count = GroupPayment.objects.filter(payment_date__date__range=[s, e], status='completed').count() if 'billing' in enabled else 0
    return {
        'services': svc, 'pharmacy': pharm, 'lab': lab,
        'revenue': total_rev, 'expenses': exp,
        'net': total_rev - exp,
        'invoices': indiv_inv_count + grp_inv_count,
        'payments': indiv_pay_count + grp_pay_count,
    }
...
def financial_reports(request):
    ...
    if 'pharmacy' in enabled:
        app_tabs.append({'key': 'pharmacy', 'label': 'Pharmacy Financials', 'icon': 'bi-capsule'})
    if 'laboratory' in enabled:
        app_tabs.append({'key': 'laboratory', 'label': 'Laboratory Revenue', 'icon': 'bi-droplet-half'})
    if 'budget' in enabled:
        app_tabs.append({'key': 'budget', 'label': 'Budget & Expenses', 'icon': 'bi-wallet2'})

    # ── Quick period stats: Today / This Week / This Month / This Year ──
    today = date.today()
    ...
    period_stats = [
        {'label': 'Today', 'stats': _quick_period_stats(today, today, enabled)},
        {'label': 'This Week', 'stats': _quick_period_stats(week_start, today, enabled)},
        {'label': 'This Month', 'stats': _quick_period_stats(month_start, today, enabled)},
        {'label': 'This Year', 'stats': _quick_period_stats(year_start, today, enabled)},
    ]

    # Gather detailed stats for the selected period (used by tab content & charts)
    billing = _get_billing_stats(start_date, end_date, prev_start, prev_end) if 'billing' in enabled else {}
    pharmacy_fin = _get_pharmacy_financial_stats(start_date, end_date) if 'pharmacy' in enabled else {}
    lab_fin = _get_laboratory_financial_stats(start_date, end_date) if 'laboratory' in enabled else {}
    budget = _get_budget_stats(start_date, end_date) if 'budget' in enabled else {}

    # General overview totals (for the selected period)
    # Total Revenue = actual payments collected (billing individual + group)
    services_revenue = float(billing.get('services_revenue', 0))
    pharmacy_revenue = float(pharmacy_fin.get('sales_revenue', 0))
    lab_revenue = float(lab_fin.get('total_revenue', 0))
    total_revenue = services_revenue
    total_expenses = float(budget.get('total_expenses', 0))
    pharmacy_cogs = float(pharmacy_fin.get('sales_cogs', 0))
    gross_profit = total_revenue - pharmacy_cogs
    net_balance = total_revenue - total_expenses

    # KPI extras
    from billing.models import GroupInvoice, GroupPayment
    outstanding_amount = float(billing.get('outstanding_amount', 0))

    # Pending invoices: individual + group
    pending_indiv = Invoice.objects.exclude(patient__patient_group__isnull=False).filter(
        created_at__date__range=[start_date, end_date], status__in=['sent', 'overdue']
    ).aggregate(cnt=Count('id'), total=Sum('total_amount')) if 'billing' in enabled else {}
    pending_grp = GroupInvoice.objects.filter(
        created_at__date__range=[start_date, end_date], status__in=['sent', 'overdue']
    ).aggregate(cnt=Count('id'), total=Sum('total_amount')) if 'billing' in enabled else {}
    pending_invoices_count = (pending_indiv.get('cnt', 0) or 0) + (pending_grp.get('cnt', 0) or 0)
    pending_invoices_amount = float(pending_indiv.get('total', 0) or 0) + float(pending_grp.get('total', 0) or 0)

    # Total completed payments in period: individual + group
    indiv_pay_agg = Payment.objects.filter(
        payment_date__date__range=[start_date, end_date], status='completed'
    ).aggregate(cnt=Count('id'), total=Sum('amount')) if 'billing' in enabled else {}
    grp_pay_agg = GroupPayment.objects.filter(
        payment_date__date__range=[start_date, end_date], status='completed'
    ).aggregate(cnt=Count('id'), total=Sum('amount')) if 'billing' in enabled else {}
    total_payments_amount = float(indiv_pay_agg.get('total', 0) or 0) + float(grp_pay_agg.get('total', 0) or 0)
    total_payments_count = (indiv_pay_agg.get('cnt', 0) or 0) + (grp_pay_agg.get('cnt', 0) or 0)

    # Gauge percentages
    collection_rate = float(billing.get('collection_rate', 0))
    budget_utilization = 0
    if budget and budget.get('active_budgets'):
        total_budgeted = sum(float(b.get('total', 0)) for b in budget['active_budgets'])
        total_spent = sum(float(b.get('spent', 0)) for b in budget['active_budgets'])
        budget_utilization = round(total_spent / total_budgeted * 100, 1) if total_budgeted else 0
    pharmacy_margin = float(pharmacy_fin.get('sales_margin', 0))
    lab_total_tests = int(lab_fin.get('total_tests', 0))
    lab_completed = int(lab_fin.get('completed_tests', 0))
    lab_completion = round(lab_completed / lab_total_tests * 100, 1) if lab_total_tests else 0
    ...
    context = {
        ...
        'gross_profit': gross_profit, 'net_balance': net_balance,
        'services_revenue': services_revenue,
        'pharmacy_revenue': pharmacy_revenue, 'lab_revenue': lab_revenue,
        'outstanding_amount': outstanding_amount,
        ...
        'pharmacy_margin': pharmacy_margin,
        'lab_completion': lab_completion,
        # Detailed tab data
        'billing': billing, 'pharmacy_fin': pharmacy_fin, 'lab_fin': lab_fin, 'budget': budget,
        # JSON for charts
        'period_stats_json': json.dumps(period_stats, default=str),
        'billing_json': json.dumps(billing, default=str) if billing else '{}',
        'pharmacy_fin_json': json.dumps(pharmacy_fin, default=str) if pharmacy_fin else '{}',
        'lab_fin_json': json.dumps(lab_fin, default=str) if lab_fin else '{}',
...
def generate_financial_export_data(parameters):
    ...
    # Recent invoices data (exclude group members)
    recent_invoices = Invoice.objects.exclude(
        patient__patient_group__isnull=False
    ).filter(
        created_at__date__range=[start_date, end_date]
...
def custom_patient_report(request):
    ...
    if 'billing' in enabled:
        inv_agg = Invoice.objects.exclude(patient__patient_group__isnull=False).filter(patient__in=patients).aggregate(total=Sum('total_amount'), cnt=Count('id'))
...
def download_custom_patient_report(request):
    ...
    if 'billing' in enabled:
        agg = Invoice.objects.exclude(patient__patient_group__isnull=False).filter(patient__in=patients).aggregate(t=Sum('total_amount'), c=Count('id'))
    ...
    if 'billing' in enabled:
        ...
        invs_base = Invoice.objects.exclude(patient__patient_group__isnull=False).filter(patient__in=patients).select_related('patient').order_by('-created_at')
...
def _apply_custom_financial_filters(request):
    ...
    # ── Billing filters (exclude group-member invoices) ──
    if 'billing' in enabled:
        invoices = Invoice.objects.exclude(patient__patient_group__isnull=False)
